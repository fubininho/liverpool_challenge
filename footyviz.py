...
def add_pass_to_fig(fig, ax, start_x, start_y, end_x, end_y, distance=False):
    plt.plot([start_x,end_x],[start_y,end_y],c='k',linewidth=2)
    if distance:
        d = np.sqrt(((start_x-end_x)**2)+((start_y-end_y)**2))
        plt.text(end_x,end_y,str(round(d,1))+" m",fontsize='x-large')
    return fig, ax

def convert_xy(point):
    return [((point[0]*100)/106)+50,((point[0]*100)/106)+50]