...
# the following code is made by fubininho
def calculate_territort_captured_by_run( event_id, player_id, events, tracking_home, tracking_away, params, GK_numbers, include_player_velocities=True, annotate=False, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    """ calculate_territort_captured_by_run
    
    # Evaluates territort captured by a player in run during an event. Calculates: total_team_score_end_event_with_run/total_team_score_end_event_without_run
    ...
    # get the details of the event with move (frame, team in possession, ball_start_position)
    ...
    checksum = np.sum( PPCFa + PPCFd ) / float(n_grid_cells_y*n_grid_cells_x ) 
    assert 1-checksum < params['model_converge_tol'], "Checksum failed: %1.3f" % (1-checksum)
    # get team score on that run
    if player_id < 15:
        #time home
        if flag_attack == "Home":
            total_team_score_end_event_with_run = np.sum(PPCFa)
        else:
            total_team_score_end_event_with_run = np.sum(PPCFd)
    else:
        if flag_attack == "Away":
            total_team_score_end_event_with_run = np.sum(PPCFa)
        else:
    ...
    # get the details of the event wihout move (frame, team in possession, ball_start_position)
    pass_frame = events.loc[event_id]['End Frame']
    pass_frame_start = events.loc[event_id]['Start Frame']
    pass_team = events.loc[event_id].Team
    ball_start_pos = np.array([events.loc[event_id]['Start X'],events.loc[event_id]['Start Y']])
    # break the pitch down into a grid
    n_grid_cells_y = int(n_grid_cells_x*field_dimen[1]/field_dimen[0])
    ...
    new_tracking_home = tracking_home.copy()
    new_tracking_away = tracking_away.copy()
    # Changing position of given player_id
    if player_id < 15:
        new_tracking_home.loc[pass_frame]["Home_"+str(player_id)+"_x"] = new_tracking_home.loc[pass_frame_start]["Home_"+str(player_id)+"_x"]
        new_tracking_home.loc[pass_frame]["Home_"+str(player_id)+"_y"] = new_tracking_home.loc[pass_frame_start]["Home_"+str(player_id)+"_y"]
    else:
        new_tracking_away.loc[pass_frame,"Away_"+str(player_id)+"_x"] = new_tracking_away.loc[pass_frame_start]["Away_"+str(player_id)+"_x"]
        new_tracking_away.loc[pass_frame,"Away_"+str(player_id)+"_y"] = new_tracking_away.loc[pass_frame_start]["Away_"+str(player_id)+"_y"]
    
    # initialise player positions and velocities for pitch control calc (so that we're not repeating this at each grid cell position)
    if pass_team=='Home':
        attacking_players = initialise_players(new_tracking_home.loc[pass_frame],'Home',params,GK_numbers[0])
        defending_players = initialise_players(new_tracking_away.loc[pass_frame],'Away',params,GK_numbers[1])
    elif pass_team=='Away':
        defending_players = initialise_players(new_tracking_home.loc[pass_frame],'Home',params,GK_numbers[0])
        attacking_players = initialise_players(new_tracking_away.loc[pass_frame],'Away',params,GK_numbers[1])
    else:
        assert False, "Team in possession must be either home or away"
    ...
    checksum = np.sum( PPCFa + PPCFd ) / float(n_grid_cells_y*n_grid_cells_x ) 
    assert 1-checksum < params['model_converge_tol'], "Checksum failed: %1.3f" % (1-checksum)
    # get team score without that run
    if player_id < 15:
        #time home
        if flag_attack == "Home":
            total_team_score_end_event_without_run = np.sum(PPCFa)
        else:
            total_team_score_end_event_without_run = np.sum(PPCFd)
    else:
        if flag_attack == "Away":
            total_team_score_end_event_without_run = np.sum(PPCFa)
        else:
    ...
    return 'The run made the team have ' + str((100*(1-total_team_score_end_event_with_run/total_team_score_end_event_without_run))) + "% more/less pitch control"